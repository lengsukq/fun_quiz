...
输出格式：必须是纯文本，不得使用 Markdown 或任何类 Markdown 符号。禁止出现：井号标题、星号/下划线加粗、反引号代码、行首减号或星号当作列表、方括号链接等。分段用空行；需要分节时，单独一行写纯文字小标题，例如「一、选择倾向」「二、可关注的点」「三、轻量建议」，其下接普通段落。若需列举，用自然口语或「第一，第二，」等，避免 Markdown 列表语法。
